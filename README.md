# Wahei Fujisaki 👋

2002年生まれのWebエンジニア。沖縄出身。福岡在住。
映像制作の世界からエンジニアになりました。  
2021年末からフリーランスとして活動しています。

## 🛠 Skills
[![My Skills](https://skillicons.dev/icons?i=vue,nuxtjs,ts,js,vite,pinia,astro,supabase,postgres,wordpress,tailwind,sass,css,html,docker,github,vercel,cloudflare,sentry,npm,pnpm,figma,xd,ai,ps,ae,obsidian,md)](https://skillicons.dev)

- WordPress オリジナルテーマ開発
- Astro + microCMS を用いた JAMstack サイト制作
- Nuxt / Vue を用いた Web アプリ開発
- Supabase / Vercel / Cloudflare Pages を用いたマネージド構成
- UI実装 / パフォーマンス改善 / CMS設計

## 🚀 Projects

### 🖼 oomisoka1231
Fashion Illustrator Portfolio Site  
WordPress オリジナルテーマで構築。  
作品が主役になる余白設計と、運用しやすいCMS構成を意識。

🔗 <a href="https://oomisoka1231.jp/" target="_blank" rel="noopener noreferrer">https://oomisoka1231.jp/</a>

---

### 🚬 喫煙ナビ
Smoking Area Search Platform  
Nuxt 4 + Supabase + Stripe + Google Maps API によるフルスタック開発。  
設計〜実装〜運用まで一人で担当。

🔗 <a href="https://smoking-navi.jp/" target="_blank" rel="noopener noreferrer">https://smoking-navi.jp/</a>

---

### 🏙 Dig&Build
Urban / Architecture Media  
Astro SSG + microCMS による高速メディアサイト。  
PageSpeed Insights：PC 98+ / Mobile 86

🔗 <a href="https://dig-and-build.com/" target="_blank" rel="noopener noreferrer">https://dig-and-build.com/</a>

## 📫 Contact
<a href="mailto:fujisakiwahei@gmail.com">fujisakiwahei@gmail.com</a>
